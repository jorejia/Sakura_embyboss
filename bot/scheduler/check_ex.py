...
from bot import bot, default_line_id, expiry_weights, group, LOGGER, _open, schedall
from bot.func_helper.emby import emby
from bot.func_helper.line_access import revoke_line_pro_access
from bot.sql_helper.sql_emby import (
    Emby,
    get_all_emby,
    sql_convert_expiry_by_weight,
    sql_update_emby,
)
...
async def check_expired():
    ...
    # 到期处理顺序：米币续期 -> 有效 Pro 权重转换 -> 禁用并进入封存期。
    ...
    for r in rst:
        if _open.exchange and r.iv >= _open.exchange_cost * 30:
            ...
            LOGGER.error(
                f'【到期检测】\n#id{r.tg} 续期账户 [{r.name}](tg://user?id={r.tg})\n'
                f'米币自动续期写入失败，继续尝试到期时间权重转换'
            )

        convert_status, converted_expiry = sql_convert_expiry_by_weight(
            r.tg,
            expiry_weights.account,
            expiry_weights.line_pro,
        )
        if convert_status == 'success':
            text = (f'【到期检测】\n#id{r.tg} 续期账户 [{r.name}](tg://user?id={r.tg})\n'
                    f'直连Pro多余时长自动续期\n'
                    f'📅统一到期: {converted_expiry.strftime("%Y-%m-%d %H:%M:%S")}')
            LOGGER.info(text)
            try:
                await bot.send_message(r.tg, text)
            except FloodWait as f:
        ...
        if convert_status == 'error':
            LOGGER.error(
                f'【到期检测】#id{r.tg} [{r.name}] 到期时间权重转换数据库操作失败，'
                f'继续执行正常到期流程'
            )
